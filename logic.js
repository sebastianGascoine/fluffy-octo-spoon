...
function getPossibleMoves(fen, detectCheck = true) {
		...
		for (let file = 0; file < 8; ++file) {
			...
			if (piece.toUpperCase() === 'P') {
				if (currentTurn === 'w') {
			        ...
			        if (!getPiece(fen, move1)) {
			            ...
			            if (!getPiece(fen, move2) && rank == 1) moves.push({ from: cell, to: move2, enPassant: move1 });
			        }
				...
				if (currentTurn === 'b') {
			        ...
			        if (!getPiece(fen, move1)) {
			            ...
			            if (!getPiece(fen, move2) && rank == 6) moves.push({ from: cell, to: move2, enPassant: move1 });
			        }
		    ...
		    if (piece.toUpperCase() === 'K') {
		        ...
		        if (getCastleOptions(fen).includes('K') && currentTurn == 'w') {
		        	if (!getPiece(fen, { rank: 0, file: 5 }) && !getPiece(fen, { rank: 0, file: 6 })) moves.push({ from: cell, to: { rank: 0, file: 6 }, castle: 'K' });
		        }
		        if (getCastleOptions(fen).includes('k') && currentTurn == 'b') {
		        	if (!getPiece(fen, { rank: 7, file: 5 }) && !getPiece(fen, { rank: 7, file: 6 })) moves.push({ from: cell, to: { rank: 7, file: 6 }, castle: 'k' });
		        }
		        if (getCastleOptions(fen).includes('Q') && currentTurn == 'w') {
		        	if (!getPiece(fen, { rank: 0, file: 1 }) && !getPiece(fen, { rank: 0, file: 2 }) && !getPiece(fen, { rank: 0, file: 3 })) moves.push({ from: cell, to: { rank: 0, file: 1 }, castle: 'Q' });
		        }
		        if (getCastleOptions(fen).includes('q') && currentTurn == 'b') {
		        	if (!getPiece(fen, { rank: 7, file: 1 }) && !getPiece(fen, { rank: 7, file: 2 }) && !getPiece(fen, { rank: 7, file: 3 })) moves.push({ from: cell, to: { rank: 7, file: 1 }, castle: 'q' });
		        }
		    ...
		    if (piece.toUpperCase() === 'B') {
		        for (let nextRank = rank - 1, nextFile = file - 1; nextRank >= 0 && nextFile >= 0; nextRank--, nextFile--) {
		            const blockingPiece = getPiece(fen, { rank: nextRank, file: nextFile });
	...
	for (let move of moves) {
		...
		if (move.castle == 'K') {
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 0, file: 5 }, 'R'));
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 0, file: 7 }, '' ));
        }
        if (move.castle == 'k') {
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 7, file: 5 }, 'r'));
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 7, file: 7 }, '' ));
        }
        if (move.castle == 'Q') {
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 0, file: 2 }, 'R'));
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 0, file: 0 }, '' ));
        }
        if (move.castle == 'q') {
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 7, file: 2 }, 'r'));
        	appliedFen = setBoard(appliedFen, setPiece(appliedFen, { rank: 7, file: 0 }, '' ));
        }

        // Disabling castling when a piece is moved.
        if (getPiece(appliedFen, move.from) == 'K') appliedFen = setCastleOptions(appliedFen, getCastleOptions(appliedFen).replace('K', '').replace('Q', ''));
        if (getPiece(appliedFen, move.from) == 'k') appliedFen = setCastleOptions(appliedFen, getCastleOptions(appliedFen).replace('k', '').replace('q', ''));

        if (getPiece(appliedFen, move.from) == 'R' && move.from.rank == 0 && move.from.file == 0) appliedFen = setCastleOptions(appliedFen, getCastleOptions(appliedFen).replace('Q', ''));
        if (getPiece(appliedFen, move.from) == 'R' && move.from.rank == 0 && move.from.file == 7) appliedFen = setCastleOptions(appliedFen, getCastleOptions(appliedFen).replace('K', ''));
        if (getPiece(appliedFen, move.from) == 'r' && move.from.rank == 7 && move.from.file == 0) appliedFen = setCastleOptions(appliedFen, getCastleOptions(appliedFen).replace('q', ''));
        ...
        for (let resultingMove of resultingMoves) {
        	if (currentTurn == 'w' && getPiece(appliedFen, resultingMove.to) == 'K') validMove = false;
        	if (currentTurn == 'b' && getPiece(appliedFen, resultingMove.to) == 'k') validMove = false;
        }
